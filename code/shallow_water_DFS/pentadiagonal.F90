module pentadiagonal
  ...
  private
  public :: pentadiagonal__allocate, pentadiagonal__ini, pentadiagonal__solve, pentadiagonal__test
...
contains
  ...
  subroutine pentadiagonal__test
...
!    ee(:) = 2.0d0
    ...
    call pentadiagonal__allocate( imax, imax+1, dat, iww )
    ...
    call pentadiagonal__ini( imax, imax+1, dat, iww )
...
!       yvar(imax) = 4.0d0
  ...
  end subroutine pentadiagonal__test
  
  
  subroutine pentadiagonal__allocate( imax, ilen, dat, iww )
...
!xx    integer,intent(out),pointer :: iww(:)
    ...
    if ( ilen < imax+1 ) then
      write(6,*) "Error: pentadiagonal__allocate: ilen should be equal to or larger than imax+1."
  ...
  end subroutine pentadiagonal__allocate


  subroutine pentadiagonal__ini( imax, ilen, dat, iww )
...
!    real(8),intent(out)   :: pp(imax)
!    real(8),intent(out)   :: qq(imax)
!    real(8),intent(out)   :: rr(imax)
!    real(8),intent(out)   :: ss(imax)
    integer,intent(out) :: iww(*)
  
    integer :: nn_tmp, nn_cyclic

    if ( ilen < imax+1 ) then
      write(6,*) "Error: pentadiagonal__ini: ilen should be equal to or larger than imax+1."
      write(6,*) "       imax, ilen = ", imax, ilen
      stop 333
    end if
  ...
  end subroutine pentadiagonal__ini


  subroutine pentadiagonal__solve  &
   &( imax, ilen, dat, iww,        &
   &  yy, yvar, xvar )
...
!    real(8),intent(in) :: aa(imax+1)
!    real(8),intent(in) :: bb(imax+1)
!    real(8),intent(in) :: cc(imax+1)
!    real(8),intent(in) :: dd(imax+1)
!    real(8),intent(in) :: ee(imax+1)
!    real(8),intent(in) :: ff(imax+1)
!    real(8),intent(in) :: pp(imax+1)
!    real(8),intent(in) :: qq(imax+1)
!    real(8),intent(in) :: rr(imax+1)
!    real(8),intent(in) :: ss(imax+1)
    ...
    if ( ilen < imax+1 ) then
      write(6,*) "Error: pentadiagonal__solve: ilen should be equal to or larger than imax+1."
      write(6,*) "       imax, ilen = ", imax, ilen
      stop 333
    end if
  ...
  end subroutine pentadiagonal__solve
  ...
  subroutine simple_solve( jmax, aa, bb, cc, dd, ee, yvar, xvar )
    integer,intent(in) :: jmax
...
!    stop 333
  ...
  !! LU factorization of block pentadiagonal matrix
  ...
  subroutine cyclic_solve( nn_cyclic, imax, aa1, bb1, cc1, dd1, ee1, ff1, pp2, qq2, rr2, ss2, jmax2_iter, jptr_iter, &
   &                                   y1, yvar, xvar )
   integer,intent(in) :: nn_cyclic
   integer,intent(in) :: imax
