...
!  integer,parameter :: JCN_MONIT_SPECTRUM = 0  !! No kinetic energy spectrum output
 ...
 ! integer,parameter :: IMAX = 2 * (8 + iz)
 ! integer,parameter :: JMAX = 8 + iz + JCN_GRID
  !integer,parameter :: NMAX = 7
 ! integer, parameter :: NMAX = JMAX - 1 - JCN_GRID
  real(8),save :: TIMESTEP = 60.0d0         !! 1 hour
!
...
! integer,parameter :: NMAX = 58            !! .9J
integer,parameter :: tr =  0           !! .95J
!real(8),save :: TIMESTEP = 3600.0d0/1.0d0      !! 1 hour
!
...
!integer,parameter :: tr =  8
!real(8),save :: TIMESTEP = 90.0d0        !! 1 hour
!
...
!integer,parameter :: NMAX = 144            !! 0.9J Quadric grid
!integer,parameter :: NMAX = 152            !!0.95J Quadric grid
!integer,parameter :: tr =  16
!real(8),parameter :: TIMESTEP =  120.0d0     !! 30 min
!
!integer,parameter :: IMAX = 640            !! 60km resolution
!integer,parameter :: JMAX = 320+JCN_GRID
!integer,parameter :: NMAX = 316            !! Linear grid
!integer,parameter :: NMAX = 213            !! Quadric grid
!integer,parameter :: NMAX = 288
!integer,parameter :: tr =  32
!real(8),parameter :: TIMESTEP = 3600.0d0     !! 20 min
!
!integer,parameter :: IMAX = 960            !! 60km resolution
!integer,parameter :: JMAX = 480+JCN_GRID
!integer,parameter :: NMAX = 472           !! Linear grid
!integer,parameter :: NMAX = 320            !! Quadric grid
!integer,parameter :: NMAX = 432
!integer,parameter :: tr =  0
!real(8),parameter :: TIMESTEP = 3600.0d0     !! 20 min
!
!integer,parameter :: IMAX = 1280           !! 60km resolution
!integer,parameter :: JMAX = 640 +JCN_GRID
!integer,parameter :: NMAX = 632      !! Linear grid
!integer,parameter :: NMAX = 427            !! Quadric grid
!integer, parameter :: NMAX = 576
!integer,parameter :: tr =  64
!real(8),parameter :: TIMESTEP = 600.0d0   !! 20 min
!
!integer, parameter :: IMAX = 1600
!integer, parameter :: JMAX = 800+JCN_GRID
!integer, parameter :: NMAX = 780           !! Linear grid
!integer, parameter :: NMAX = 533            !! Quadratic grid
!integer, parameter :: tr = 0
!real(8), parameter :: TIMESTEP = 60.0d0       !! 10 min
...
! Initialize for dfs Interpolation
  ...
  real(8), save :: ud_halo(JMAX,IMAX),vd_halo(JMAX,IMAX),hd_halo(JMAX,IMAX),wd_halo(JMAX,IMAX)
  real(8), save :: th_d(IMAX*JMAX) ! departure 1d
  real(8), save :: lb_d(IMAX*JMAX)
  complex*16, save :: coeffs2d(2*(JMAX-JCN_GRID),IMAX), coeffs1d(2*(JMAX-JCN_GRID)*IMAX)
  complex*16, save :: cn(2*(JMAX-JCN_GRID))
  real(8), save:: tmph(IMAX*JMAX),tmpu(IMAX*JMAX),tmpv(IMAX*JMAX),tmpw(IMAX*JMAX)
  integer :: tmpindex, iret
  integer(8) :: N1, N2
  real(8) :: vij, uij, timemany
  
  real(8) :: tgrid, tvals, tfin, tcopv,tcoeff, tstart, tfinal
  real(8) :: tmpcosd
!
!   FINUFFT declarations
  integer :: ier, iflag, ntrans, dim, ttype
  integer(8) :: M, plan, planF
  integer(8), allocatable :: n_modes(:)
  complex*16, save :: fu(IMAX*JMAX)
...
!
  ...
  nstep = int(kt_end*((3600.0d0)/TIMESTEP)) + nstep_start
!
  !intv_monit = INTHR_MONIT*ntimes_hour
  intv_monit = int(INTHR_MONIT*(3600.0d0/TIMESTEP))
...
!
  ...
  ! set mandotary parameters FINUFFT guru interface
    ...
    n_modes(1) = N1
    n_modes(2) = N2
    !tr = (JMAX-JCN_GRID) - 1* (JMAX-JCN_GRID) 
    print*, "tr = ", tr
    print*, "                     "
  ...
  call dfftw_plan_with_nthreads(omp_get_num_threads())
   call dfftw_plan_dft_2d(planF, N1,N2,coeffs2d,coeffs2d,&
       &                       FFTW_FORWARD, FFTW_ESTIMATE)
  ! FFTW3 shifting coefficients
    call shiftfftw(JCN_GRID, JMAX, IMAX, cn)
  !
    call finufft_makeplan(ttype,dim,n_modes,iflag,ntrans,&
    & tol,plan,opts,ier)
    timemany = omp_get_wtime()
    call finufft_setpts(plan,M,th_d,lb_d,dummy,dummy,&
...
!
  ...
  ! 
      ...
      th_d = reshape(tt, [IMAX*JMAX])
      lb_d = reshape(ll, [IMAX*JMAX])
...
!   interpolate u
   ...
   ! &     dummy,dummy,dummy,ier)
    !
    call doubleUp(ud_halo, coeffs2d, JCN_GRID)
    call vals2coeffs(coeffs2d, cn, planF, tr)
    coeffs1d = reshape(coeffs2d, [N1*N2])
    call finufft_execute(plan,fu,coeffs1d,ier)
    tmpu = dble(fu)
!
!   interpolate v
    call doubleUp(vd_halo, coeffs2d, JCN_GRID)
    call vals2coeffs(coeffs2d, cn, planF, tr)
    coeffs1d = reshape(coeffs2d, [N1*N2])
    call finufft_execute(plan,fu,coeffs1d,ier)
    tmpv = dble(fu)
...
!   interpolate h
    call doubleUp(hd_halo, coeffs2d, JCN_GRID)
    call vals2coeffs(coeffs2d, cn, planF, tr)
    coeffs1d = reshape(coeffs2d, [N1*N2])
    call finufft_execute(plan,fu,coeffs1d,ier)
    tmph = dble(fu)
    timemany = omp_get_wtime()-timemany
    
     if(VELOCITYD .eq. 3) then
     !   interpolate w
    call doubleUp(wd_halo, coeffs2d, JCN_GRID)
    call vals2coeffs(coeffs2d, cn, planF, tr)
    coeffs1d = reshape(coeffs2d, [N1*N2])
    call finufft_execute(plan,fu,coeffs1d,ier)
    tmpw = dble(fu)
!
      ...
      do j=1, JMAX
        do i=1, IMAX
          tmpindex = (j-1)*IMAX + i
          uij = DCOS(lb_d(tmpindex))*tmpv(tmpindex)-DSIN(lb_d(tmpindex))*tmpu(tmpindex)
          vij = DCOS(th_d(tmpindex))*tmpw(tmpindex)-DSIN(lb_d(tmpindex))*DSIN(th_d(tmpindex))*tmpv(tmpindex)& 
   &         -DCOS(lb_d(tmpindex))*Dsin(th_d(tmpindex))*tmpu(tmpindex)
...
!
      ...
      do i=i1,i2
       ...
       ! yj(i) = -tt(i)/DLAT + (JMAX+1)*0.5d0
        ...
        !if(iter == iterx) then
        tt(i) = alatd 
       ! ll(i) = mod(xi(i), 1D0*IMAX)*DLON
        ll(i) = ALON(i) + atan2(sindlond,cosdlond) 
        !if((tt(i)< -PI/2) .or. (tt(i) > PI/2)) then
...
subroutine dfsInterp(coeffsh, coeffsu, coeffsv, coeffsw,&
  & u,v,w, h, N1, N2, th, lb)
      complex*16, intent(in) :: coeffsh(:),coeffsu(:),coeffsv(:)
      complex*16, intent(in) :: coeffsw(:)
      real*8, intent(in) :: lb(:), th(:)
      integer(8), intent(in) :: N1, N2
      real(8), intent(out), allocatable :: u(:), v(:), w(:), h(:)
      integer :: ier, iflag, ntrans, dim, ttype
      integer*8 :: M, plan
      integer(8), allocatable :: n_modes(:)
      real(8) :: tol
      complex*16, allocatable :: fu(:), fv(:), fw(:), fh(:)
      real(8), pointer :: dummy => null()
...
!    set mandatory parameters to FINUFFT guru interface
      iflag = 1
      tol = 1d-14
      ntrans = 1
      ttype = 2
   ...
   ! call finufft2d2(M,th,lb,fu,iflag,tol,N1,N2,coeffsu,opts,ier)
   ! call finufft2d2(M,th,lb,fv,iflag,tol,N1,N2,coeffsv,opts,ier)
  ! call finufft2d2(M,th,lb,fw,iflag,tol,N1,N2,coeffsw,opts,ier)
 !   call finufft2d2(M,th,lb,fh,iflag,tol,N1,N2,coeffsh,opts,ier)
 !  use default options
   call finufft_makeplan(ttype,dim,n_modes,iflag,ntrans,&
  &     tol,plan,opts,ier)
    
!  note for type 1 or 2, arguments 6-9 ignored...
  call finufft_setpts(plan,M,th,lb,dummy,dummy,&
 &     dummy,dummy,dummy,ier)

 call finufft_execute(plan,fh,coeffsh,ier)
 call finufft_execute(plan,fu,coeffsu,ier)
 call finufft_execute(plan,fv,coeffsv,ier)
 call finufft_execute(plan,fw,coeffsw,ier)
 call finufft_destroy(plan,ier)
...
!==========================================================================
  subroutine dfsInterp1(coeffsh, h, N1, N2, th, lb)
        complex*16, intent(in) :: coeffsh(:)
        real*8, intent(in) :: lb(:), th(:)
        integer(8), intent(in) :: N1, N2
        complex(8), intent(inout) :: h(:)
        integer :: ier, iflag, ntrans
        integer*8 :: M
        real(8) :: tol
        ...
        tol = 1D-14
        !
        call finufft2d2many(ntrans, M, th, lb, h, iflag,&
        &   tol,N1, N2, coeffsh, opts, ier)
        !

        return 
    end subroutine dfsInterp1
