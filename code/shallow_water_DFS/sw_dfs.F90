...
!  integer,parameter :: JCN_HDIFF = 2  !! 4th order hyper diffusion
  ...
  !integer:: JCN_ZONALFILTER = 0 !! No zonal filter
...
! real(8),save :: TIMESTEP = 3600      !! 1 hour
!
...
!integer, parameter :: trunc = 8
 !real(8),save :: TIMESTEP = 3600.0d0/8d0        !! 1 hour
!
...
!integer, parameter :: trunc = 
!real(8),parameter :: TIMESTEP = 120.00d0   !! 30 min
!
...
!integer, parameter :: trunc = 32
real(8),parameter :: TIMESTEP = 1800.0d0   !! 20 min.
 !
!integer,parameter :: IMAX = 960            !!
!integer,parameter :: JMAX = 480+JCN_GRID
!integer,parameter :: NMAX = 472           !!
!integer,parameter :: NMAX = 320            !!
!integer,parameter :: NMAX = 432
!integer,parameter :: trunc =  48
!real(8),parameter :: TIMESTEP = 3600.0d0
 !
...
!real(8), parameter :: TIMESTEP = 60.0d0       !! 10 min
 ...
 integer,parameter :: NMAX = 948            !! Linear grid
 !integer,parameter :: NMAX = 640            !! Quadric grid
...
! Initialize for dfs Interpolation
  real(8), save :: lld(IMAX,JMAX), ttd(IMAX,JMAX)
  real(8) :: ud_halo(IMAX,JMAX), vd_halo(IMAX,JMAX), hd_halo(IMAX,JMAX)
  real(8) :: th_d(IMAX*JMAX), lb_d(IMAX*JMAX)
  real(8) :: tmph((IMAX*JMAX)), tmpu((IMAX*JMAX)), tmpv((IMAX*JMAX))
  complex*16 :: coeffs_u(((2*NMAX+1)*(2*NMAX+1))), fu(IMAX*JMAX), cn(2*NMAX+1)
  complex*16 :: coeffs_v(((2*NMAX+1)*(2*NMAX+1))), coeffs_h(((2*NMAX+1)*(2*NMAX+1)))
  !!
  integer :: tmpindex
  integer(8) :: N1, N2
  integer(8) :: nmodes(3)
...
!
  ...
  ! set mandotary parameters FINUFFT guru interface
   ...
   ! print* , "start her
    ...
    nmodes(2) = N2
  !
    call fshisfts(cn, NMAX, JCN_GRID)
...
! interpolating using dfs method 
    ...
    !$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i,j,tmpindex) SCHEDULE(STATIC)
    do tmpindex = 1, IMAX*JMAX
      j = (tmpindex - 1)/IMAX + 1
      i = tmpindex - (j-1)*IMAX
      ud_halo(i,j) = u_halo(i,j)
      vd_halo(i,j) = v_halo(i,j)
      hd_halo(i,j) = phi_halo(i,j)
    end do
  !$OMP END PARALLEL DO
!
    call set_halo_uv &
...
!   interpolate u and v
   ...
   !  &     dummy,dummy,dummy,ier)
    ...
    do j = 1, JMAX
      do i = 1, IMAX
        tmpindex = (j-1)*IMAX + i
        th_d(tmpindex) = ttd(i,j)
        lb_d(tmpindex) = lld(i,j)
      end do
  end do
  !$OMP END DO
  !$OMP END PARALLEL
      ...
      call  newdfs_complex_uv(ud_halo, vd_halo,  coeffs_u, coeffs_v, NMAX, cn)
      !call finufft2d2(M,th_d,lb_d,fu,iflag,tol,N1,N2,coeffs_u,opts,ier)
      call finufft_execute(plan,fu,coeffs_u,ier)
      
     
      tmpu = dble(fu)
     ! print*, "MAX THREADS", omp_get_max_threads()
     
      call finufft_execute(plan,fu,coeffs_v,ier)
     ! call finufft2d2(M,th_d,lb_d,fu,iflag,tol,N1,N2,coeffs_v,opts,ier)
      tmpv = dble(fu)
    
      call  newdfs_complex_h(hd_halo, coeffs_h, NMAX, cn)
      call finufft_execute(plan,fu,coeffs_h,ier)
     ! call finufft2d2(M,th_d,lb_d,fu,iflag,tol,N1,N2,coeffs_h,opts,ier)
      tmph = dble(fu)
      ...
      do tmpindex = 1, IMAX*JMAX
        j = (tmpindex - 1)/IMAX + 1
        i = tmpindex - (j-1)*IMAX
        u0(i,j)   = cosdtheta(i,j)*tmpu(tmpindex) - sindtheta(i,j)*tmpv(tmpindex)
        v0(i,j)   = sindtheta(i,j)*tmpu(tmpindex) + cosdtheta(i,j)*tmpv(tmpindex)
