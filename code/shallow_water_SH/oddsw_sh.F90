...
! ===================================================================
  ...
  if(IT .eq. 1) then
    N1 = JMAX * 2
    N2 = IMAX
    allocate(thd(JMAX*IMAX))
    allocate(lbd(JMAX*IMAX))
    allocate(th(N1))
    allocate(udhalo(JMAX, IMAX))
    allocate(vdhalo(JMAX, IMAX))
    allocate(hdhalo(JMAX, IMAX))
    allocate(wdhalo(JMAX, IMAX))
    allocate(coeffs2d(N1,N2))
    allocate(coeffs1d(N1*N2))
    allocate(tmph(IMAX*JMAX))
    allocate(tmpu(IMAX*JMAX))
    allocate(tmpw(IMAX*JMAX))
    allocate(tmpv(IMAX*JMAX))
    allocate(ipiv(N1))
    allocate(L(N1,N2))
    allocate(cn(N1,N2))
    allocate(fu(IMAX*JMAX))
!
...
!
   ...
   !$OMP END PARALLEL
!
    thd = reshape(tt, [IMAX*JMAX])
    lbd = reshape(ll-2*pi, [IMAX*JMAX])
!
!   interpolate u
